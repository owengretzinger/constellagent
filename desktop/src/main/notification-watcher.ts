...
interface NotificationCallbacks {
  ...
  activityUpdate: (workspaceIds: string[]) => void;
}
...
export class NotificationWatcher {
  ...
  private processAgentEventFile(filePath: string): void {
    try {
      ...
      if (!raw) { unlinkSync(filePath); return; }
  ...
  private normalizeEvent(parsed: Partial<AgentTurnEvent>): AgentTurnEvent | null {
    ...
    const agent = typeof parsed.agent === "string" && parsed.agent.trim() ? parsed.agent.trim() : "unknown";
    const sessionId = typeof parsed.sessionId === "string" && parsed.sessionId.trim() ? parsed.sessionId.trim() : undefined;
    const outcome = typeof parsed.outcome === "string" && isTurnOutcome(parsed.outcome) ? parsed.outcome : undefined;
    return {
      schema: 1,
      workspaceId,
      type,
  ...
  private removeActiveSession(workspaceId: string, agent: string, sessionId?: string): void {
    ...
    if (sessions.size === 0) this.activeEventSessions.delete(workspaceId);
  }
  ...
  private publishActivity(): void {
    ...
    this.callbacks?.activityUpdate(Array.from(nextActive).sort());

    for (const wsId of becameInactive) {
      this.callbacks?.notifyWorkspace(wsId);
    }
  }
